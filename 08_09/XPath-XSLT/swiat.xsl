...
<!--          <xsl:apply-templates select="SWIAT/KRAJE/KRAJ[@KONTYNENT='k1']"/>-->
          <xsl:apply-templates select="SWIAT/KRAJE/KRAJ[@KONTYNENT=//KONTYNENTY/KONTYNENT[NAZWA='Europe']/@ID]">
            <xsl:sort select="NAZWA"/>
          </xsl:apply-templates>
        </table>
        Liczba krajów: <xsl:value-of select="count(//SWIAT/KRAJE/KRAJ[@KONTYNENT=//KONTYNENTY/KONTYNENT[NAZWA='Europe']/@ID])"/>
      </body>
    </html>
  </xsl:template>
  <xsl:template match="*">
